...
test('2 — CONVERSATION tab: rich text editor renders', async ({ browser }) => {
  ...
  await page.locator('[data-testid="activity-filter-comments"]').click();
  await page.waitForTimeout(500);

  // Verify the rich text editor textarea is present
  const textarea = page.locator('[data-testid="activity-comment-input"] textarea');
...
test('3 — CONVERSATION tab: type and post a comment', async ({ browser }) => {
  ...
  const textarea = page.locator('[data-testid="activity-comment-input"] textarea').first();
  ...
  const commentList = page.locator('.ea-comment-list');
...
test('5 — NOTES tab: rich text editor + add note', async ({ browser }) => {
  ...
  await page.locator('[data-testid="activity-filter-notes"]').click();
  await page.waitForTimeout(500);

  // Verify textarea visible
  const textarea = page.locator('[data-testid="activity-note-input"] textarea');
  await expect(textarea).toBeVisible();
  ...
  const richDisplay = page.locator('.ea-note-item app-rich-text-display');
...
test('6 — NOTES tab: existing notes render as rich text (not plain text)', async ({ browser }) => {
  ...
  const noteItems = page.locator('.ea-note-item');
  const noteCount = await noteItems.count();
  console.log(`Note items: ${noteCount}`);

  if (noteCount > 0) {
    const display = page.locator('.ea-note-item app-rich-text-display');
...
test('7 — ALL tab: merged feed shows both comments and notes', async ({ browser }) => {
  ...
  await page.locator('[data-testid="activity-filter-all"]').click();
  await page.waitForTimeout(500);
...
test('8 — HISTORY tab: renders without error', async ({ browser }) => {
  ...
  await page.locator('[data-testid="activity-filter-history"]').click();
  ...
  const emptyHint = page.locator('.ea-empty');
  const timeline = page.locator('app-activity-timeline');
