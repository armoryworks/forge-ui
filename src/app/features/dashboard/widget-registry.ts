import { DashboardWidgetConfig } from './models/dashboard-widget-config.model';

export const WIDGET_REGISTRY: DashboardWidgetConfig[] = [
  // ─── Row 0: primary operational widgets (3 × 33%) ───
  {
    id: 'todays-tasks',
    title: "Today's Tasks",
    icon: 'task_alt',
    component: 'todays-tasks',
    capability: 'CAP-EXT-KANBAN',
    defaultX: 0,
    defaultY: 0,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
    viewAllLink: '/kanban',
    viewAllLabel: 'View board',
  },
  {
    id: 'jobs-by-stage',
    title: 'Jobs by Stage',
    icon: 'bar_chart',
    component: 'jobs-by-stage',
    capability: 'CAP-EXT-KANBAN',
    defaultX: 4,
    defaultY: 0,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 3,
    viewAllLink: '/kanban',
    viewAllLabel: 'View board',
  },
  {
    id: 'team-load',
    title: 'Team Load',
    icon: 'groups',
    component: 'team-load',
    capability: 'CAP-EXT-KANBAN',
    defaultX: 8,
    defaultY: 0,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 3,
    viewAllLink: '/time-tracking',
    viewAllLabel: 'Time tracking',
  },
  // ─── Row 1: deadlines + activity + margin (3 × 33%) ───
  {
    id: 'deadlines',
    title: 'Deadlines',
    icon: 'event',
    component: 'deadlines',
    capability: 'CAP-EXT-KANBAN',
    defaultX: 0,
    defaultY: 4,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
    viewAllLink: '/calendar',
    viewAllLabel: 'View calendar',
  },
  {
    id: 'activity',
    title: 'Activity',
    icon: 'history',
    component: 'activity',
    defaultX: 4,
    defaultY: 4,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
  },
  {
    id: 'margin-summary',
    title: 'Margin Summary',
    icon: 'trending_up',
    component: 'margin-summary',
    capability: 'CAP-ACCT-BUILTIN',
    defaultX: 8,
    defaultY: 4,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
  },
  // ─── Row 2: cycle + orders + eod (3 × 33%) ───
  {
    id: 'cycle-progress',
    title: 'Cycle Progress',
    icon: 'loop',
    component: 'cycle-progress',
    capability: 'CAP-PLAN-MRP',
    defaultX: 0,
    defaultY: 8,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
    viewAllLink: '/planning',
    viewAllLabel: 'Planning cycles',
  },
  {
    id: 'open-orders',
    title: 'Open Orders',
    icon: 'shopping_cart',
    component: 'open-orders',
    capability: 'CAP-O2C-SO',
    defaultX: 4,
    defaultY: 8,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
    viewAllLink: '/sales-orders',
    viewAllLabel: 'All orders',
  },
  {
    id: 'eod-prompt',
    title: 'End of Day',
    icon: 'nightlight',
    component: 'eod-prompt',
    capability: 'CAP-HR-TIMETRACK',
    defaultX: 8,
    defaultY: 8,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
  },
  // ─── Row 3: action items (follow-up tasks) ───
  {
    id: 'action-items',
    title: 'Action Items',
    icon: 'task_alt',
    component: 'action-items',
    defaultX: 0,
    defaultY: 12,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
  },
  // ─── Inventory snapshot — the lead widget when only Inventory is enabled ───
  {
    id: 'low-stock',
    title: 'Inventory Snapshot',
    icon: 'inventory_2',
    component: 'low-stock',
    capability: 'CAP-INV-CORE',
    defaultX: 0,
    defaultY: 16,
    defaultW: 4,
    defaultH: 4,
    minW: 2,
    minH: 2,
    viewAllLink: '/inventory/home/tasks',
    viewAllLabel: 'View inventory',
  },
];
